    ...
    public class UserObject
    {
        ...
        public void CreateUserTable(string UserTableName, string UserTableDesc, BoUTBTableType UserTableType)
        {
            ...
            try
            {
                ...
                this.ValidateAction(UserTableName);
            }
            catch (Exception ex)
            {
                LogList.Add($"Erro geral ao criar tabela {UserTableName}: {ex.Message}");
            }
            finally
            {
        ...
        public void InsertUserField(string TableName, string FieldName, string FieldDescription, BoFieldTypes oType, BoFldSubTypes oSubType, int FieldSize, bool MandatoryYN = false, string DefaultValue = "", string linkedTable = "", string linkedUDO = "")
        {
            if (Confirmed == false)
            {
                return;
            ...
            bool ok = oUserFieldsMD.GetByKey("@AXT_OFAC", 28);
            ...
            this.ValidateAction(TableName, FieldName);
        ...
        public void CreateUserObject(string ObjectName, string ObjectDesc, string TableName, BoUDOObjType ObjectType, bool CanLog, bool CanDelete, bool CanFind, bool CanYearTransfer, bool CanCancel, bool CanClose, bool CanCreateDefaultForm, int FatherMenuId, int menuPosition, string srfFile = "")
        {
            ...
            try
            {
                ...
                this.ValidateAction(ObjectName);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void ValidateAction(string table = "", string field = "")
        {
            if (CodErro != 0)
            {
                SBOApp.Company.GetLastError(out CodErro, out MsgErro);
                LogList.Add($"FALHA ({table} - {field}) - {MsgErro}");
            }
            else
            {
                MsgErro = "";
        ...
        public void MakeFieldsSearchable(string tableName)
        {
            ...
            if (userObjectsMD.GetByKey(tableName))
            {
                ...
                foreach (var item in fields)
                {
                    ...
                    {
                        hasNewColumn = true;
                        if (!String.IsNullOrEmpty(userObjectsMD.FindColumns.ColumnAlias))
                        {
                            userObjectsMD.FindColumns.Add();
                        }
